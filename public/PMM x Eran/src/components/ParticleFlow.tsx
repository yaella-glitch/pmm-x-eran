...
const FEATURE_LOGO_X = 130;

const FUNNEL_TOP_LEFT_X = 720;
const FUNNEL_TOP_RIGHT_X = 1140;
const FUNNEL_BOT_LEFT_X = 850;
const FUNNEL_BOT_RIGHT_X = 1020;
const FUNNEL_TOP_Y = 200;
const FUNNEL_BOT_Y = 660;
...
export default function ParticleFlow() {
  ...
  const zones = [
    { key: "top" as const, label: "Top of funnel", yStart: FUNNEL_TOP_Y, yEnd: FUNNEL_TOP_Y + zoneHeight },
    { key: "middle" as const, label: "Middle", yStart: FUNNEL_TOP_Y + zoneHeight, yEnd: FUNNEL_TOP_Y + 2 * zoneHeight },
    { key: "bottom" as const, label: "Bottom", yStart: FUNNEL_TOP_Y + 2 * zoneHeight, yEnd: FUNNEL_BOT_Y },
  ];
  ...
  return (
    ...
    >
      ...
      {/* PERSONAS — small avatars with tiny labels entering the funnel */}
      {personas && personas.length > 0 && (() => {
        const personaY = 100;
        const personaSpacing = (FUNNEL_TOP_RIGHT_X - FUNNEL_TOP_LEFT_X) / (personas.length + 1);
        const funnelTopMidX = (FUNNEL_TOP_LEFT_X + FUNNEL_TOP_RIGHT_X) / 2;
        return (
          <g>
            {personas.map((p, i) => {
              const px = FUNNEL_TOP_LEFT_X + personaSpacing * (i + 1);
              const targetX = funnelTopMidX;
              const path = `M ${px} ${personaY + 16} L ${targetX} ${FUNNEL_TOP_Y - 4}`;
              return (
                <g key={`persona-${i}`}>
                  <line
                    x1={px}
      ...
      {/* IN: feature → heart curves + particles + logo ONLY (no text) */}
      {features.map((f, i) => {
        const fy = featStartY + i * featSpacing;
        const lineStart = FEATURE_LOGO_X + 22;
            ...
            <g>
              ...
              {f.logo ? (
                ...
                <text
                  ...
                  fontWeight={500}
                >
                  {f.name.replace("monday ", "").charAt(0).toUpperCase()}
                </text>
              )}
              <title>{f.name}</title>
            </g>
      ...
      <g>
        ...
        <text x={HEART_X} y={HEART_Y - 2} textAnchor="middle" fontFamily="Figtree, sans-serif" fontSize="24" fontWeight={500} fill="#a58aff">
          PMM
        </text>
        <text x={HEART_X} y={HEART_Y + 18} textAnchor="middle" fontFamily="Crimson Text, serif" fontStyle="italic" fontSize="12" className="svg-text-soft">
          translates value
        </text>
      </g>

      {/* OUT: heart → funnel (one line per zone) */}
      {zones.map((z, zi) => {
      ...
      <g>
        ...
        {zones.map((z, zi) => {
          ...
          const labelY = z.yStart + 24;
          const leftX = funnelXAtY(labelY, "left");
          const itemStartY = labelY + 22;
          return (
            <g key={`zone-${zi}`}>
              <text
                x={leftX + 20}
      ...
      {/* Legend bottom-left */}
      <g transform={`translate(${FEATURE_LOGO_X - 60}, ${H - 16})`}>
