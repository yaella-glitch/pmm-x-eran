    ...
    <>
      <button
        ...
        style={{
          ...
          cursor: "pointer",
          color: hasOverride ? "white" : "var(--color-text)",
          fontFamily: "inherit",
        }}
      >
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <Section title="1. Hero">
        <TextField label="Section label" value={draft.section1_hero.label} onChange={(v) => update(["section1_hero", "label"], v)} />
        <TextField label="Title" value={draft.section1_hero.title} onChange={(v) => update(["section1_hero", "title"], v)} />
        <TextField label="Date" value={draft.section1_hero.date} onChange={(v) => update(["section1_hero", "date"], v)} />
      </Section>

      {/* AGENDA */}
      <Section title="2. What do we want to achieve today">
        <TextField label="Section label" value={draft.section2_agenda.label} onChange={(v) => update(["section2_agenda", "label"], v)} />
        <TextAreaField label="Main statement" value={draft.section2_agenda.statement} onChange={(v) => update(["section2_agenda", "statement"], v)} />
        <TextAreaField label="Italic tagline (small, below statement)" value={draft.section2_agenda.tagline} onChange={(v) => update(["section2_agenda", "tagline"], v)} />
        <ListField
          label="Agenda items"
          items={draft.section2_agenda.agenda}
          onChange={(i, v) => update(["section2_agenda", "agenda", i], v)}
          onAdd={() => arrayAdd(["section2_agenda", "agenda"], "New item")}
          onRemove={(i) => arrayRemove(["section2_agenda", "agenda"], i)}
        />
      </Section>

      {/* PMM ROLE */}
      <Section title="3. The PMM role tl;dr">
        ...
        <TextField label="Subtitle" value={draft.section3_what_is_pmm.subtitle} onChange={(v) => update(["section3_what_is_pmm", "subtitle"], v)} />
          ...
          {draft.section3_what_is_pmm.personas?.map((p: any, i: number) => (
            <Row key={i}>
              <TextField
                label={`Persona ${i + 1}`}
                value={p.name}
                onChange={(v) => update(["section3_what_is_pmm", "personas", i, "name"], v)}
              />
              <RemoveBtn onClick={() => arrayRemove(["section3_what_is_pmm", "personas"], i)} />
            </Row>
          ))}
          <AddBtn onClick={() => arrayAdd(["section3_what_is_pmm", "personas"], { name: "New" })}>+ Add persona</AddBtn>
        </Subgroup>
          ...
          {draft.section3_what_is_pmm.features.map((f: any, i: number) => (
            <Row key={i}>
              <TextField
                label={`Feature ${i + 1}`}
                value={f.name}
                onChange={(v) => update(["section3_what_is_pmm", "features", i, "name"], v)}
              />
              <TextField
                label="Logo URL (optional)"
                value={f.logo}
                onChange={(v) => update(["section3_what_is_pmm", "features", i, "logo"], v)}
              />
              <RemoveBtn onClick={() => arrayRemove(["section3_what_is_pmm", "features"], i)} />
            </Row>
          ))}
          <AddBtn onClick={() => arrayAdd(["section3_what_is_pmm", "features"], { name: "New", logo: "" })}>+ Add feature</AddBtn>
        </Subgroup>

        <Subgroup title="Funnel outputs — Top of funnel">
          <OutputsEditor
        ...
        <Subgroup title="Funnel outputs — Middle">
          <OutputsEditor
        ...
        <Subgroup title="Funnel outputs — Bottom">
          <OutputsEditor
      ...
      <Section title="4. A glimpse to our deliverables">
        ...
        <TextField label="Title (shown above)" value={draft.section4_deliverables.title} onChange={(v) => update(["section4_deliverables", "title"], v)} />

        <Subgroup title="Deliverables (carousel cards)">
          {draft.section4_deliverables.items.map((it: any, i: number) => (
            <NestedCard key={i} title={`Deliverable ${i + 1}: ${it.name || "(unnamed)"}`} onRemove={() => arrayRemove(["section4_deliverables", "items"], i)}>
              <TextField label="Name" value={it.name} onChange={(v) => update(["section4_deliverables", "items", i, "name"], v)} />
              <TextField label="Image path (e.g. /images/file.png)" value={it.image} onChange={(v) => update(["section4_deliverables", "items", i, "image"], v)} hint="Drop the image into public/images/ first, then put the path here." />
              <TextField label="KPI (big number/value)" value={it.kpi} onChange={(v) => update(["section4_deliverables", "items", i, "kpi"], v)} />
              <TextField label="KPI detail (small text)" value={it.kpi_detail} onChange={(v) => update(["section4_deliverables", "items", i, "kpi_detail"], v)} />
            </NestedCard>
          ))}
          <AddBtn onClick={() => arrayAdd(["section4_deliverables", "items"], { name: "New deliverable", image: "", kpi: "—", kpi_detail: "" })}>+ Add deliverable</AddBtn>
        </Subgroup>
      </Section>

      {/* CHALLENGES */}
      <Section title="5. PMM impact on marketing goals">
        <TextField label="Section label" value={draft.section5_challenges.label} onChange={(v) => update(["section5_challenges", "label"], v)} />
        <TextField label="Title" value={draft.section5_challenges.title} onChange={(v) => update(["section5_challenges", "title"], v)} />

        <Subgroup title="Challenge → Solution rows">
          {draft.section5_challenges.rows.map((r: any, i: number) => (
            <NestedCard key={i} title={`Row ${i + 1}`} onRemove={() => arrayRemove(["section5_challenges", "rows"], i)}>
              <TextAreaField label="Challenge" value={r.challenge} onChange={(v) => update(["section5_challenges", "rows", i, "challenge"], v)} />
              <TextAreaField label="PMM solution" value={r.solution} onChange={(v) => update(["section5_challenges", "rows", i, "solution"], v)} />
            </NestedCard>
          ))}
          <AddBtn onClick={() => arrayAdd(["section5_challenges", "rows"], { challenge: "New challenge", solution: "How PMM solves it" })}>+ Add row</AddBtn>
        </Subgroup>
      </Section>

      {/* MUST-WIN BATTLES */}
      <Section title="6. Must-win battles">
        <TextField label="Section label" value={draft.section6_battles.label} onChange={(v) => update(["section6_battles", "label"], v)} />
        <TextField label="Title" value={draft.section6_battles.title} onChange={(v) => update(["section6_battles", "title"], v)} />

        {draft.section6_battles.battles.map((b: any, i: number) => (
          <NestedCard key={i} title={`Battle ${i + 1}: ${b.name}`} onRemove={() => arrayRemove(["section6_battles", "battles"], i)}>
            <TextField label="Name" value={b.name} onChange={(v) => update(["section6_battles", "battles", i, "name"], v)} />
            <TextField label="Summary (shown on card)" value={b.summary} onChange={(v) => update(["section6_battles", "battles", i, "summary"], v)} />

            <Subgroup title="Sub-views (tabs inside the card)">
              {b.subviews.map((sv: any, j: number) => (
                <NestedCard key={j} title={`Tab ${j + 1}: ${sv.name}`} onRemove={() => arrayRemove(["section6_battles", "battles", i, "subviews"], j)}>
                  <TextField label="Tab name" value={sv.name} onChange={(v) => update(["section6_battles", "battles", i, "subviews", j, "name"], v)} />
                  {sv.body !== undefined && (
                    <TextAreaField label="Body text" value={sv.body} onChange={(v) => update(["section6_battles", "battles", i, "subviews", j, "body"], v)} />
                  )}
                  {sv.kind === "baseline" && (
                    <BaselineEditor
                      subview={sv}
                      path={["section6_battles", "battles", i, "subviews", j]}
                      update={update}
                      arrayAdd={arrayAdd}
                      arrayRemove={arrayRemove}
                    />
      ...
      <Section title="7. Closing">
        <TextField label="Section label" value={draft.section7_closing.label} onChange={(v) => update(["section7_closing", "label"], v)} />
    ...
    <>
      {subview.stages.map((stage: any, i: number) => (
        <NestedCard key={i} title={`Stage ${i + 1}: ${stage.name}`} onRemove={() => arrayRemove([...path, "stages"], i)}>
          ...
          <TextField label="Asset image path (optional)" value={stage.asset || ""} onChange={(v) => update([...path, "stages", i, "asset"], v)} hint="Drop image into public/images/ then use /images/yourfile.png" />
        </NestedCard>
      ))}
  ...
  return (
    <label style={{ display: "flex", flexDirection: "column", gap: "0.3rem" }}>
      ...
      <textarea
        ...
        onChange={(e) => onChange(e.target.value)}
        rows={2}
        style={{ ...inputStyle, resize: "vertical", minHeight: 48, lineHeight: 1.4 }}
      />
    </label>
  );
}
...
function ListField({ label, items, onChange, onAdd, onRemove }: { label: string; items: string[]; onChange: (i: number, v: string) => void; onAdd: () => void; onRemove: (i: number) => void }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
      <FieldLabel>{label}</FieldLabel>
      {items.map((item, i) => (
        <Row key={i}>
          <TextField label="" value={item} onChange={(v) => onChange(i, v)} />
          <RemoveBtn onClick={() => onRemove(i)} />
        </Row>
      ))}
      <AddBtn onClick={onAdd}>+ Add item</AddBtn>
    </div>
  );
}
